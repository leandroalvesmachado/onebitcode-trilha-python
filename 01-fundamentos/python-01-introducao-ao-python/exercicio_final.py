...
Adicionar um time, Remover um time, Listar times, Adicionar jogador em um time,
Remover jogador de um time, Listar jogadores de um time.

1 - A opção de listar os times deve mostrar o índice, o nome e
a quantidade de jogadores do time.

2 - A opção de adicionar um time deve pedir um nome
para o time que será cadastrado.

3 - A opção de remover um time deve pedir o índice específico do time
que foi cadastrado para fazer a sua exclusão.

4 - A opção de adicionar um jogador em um time deve pedir um índice do
time que foi cadastrado e associar com o nome do jogador que será adicionado.

5 - A opção de remover um jogador em um time deve pedir um índice do time
que foi cadastrado e utilizar esse índice para remover o jogador que
fora cadastrado no time.

6 - A opção de listar os jogadores de um time deve ser informado o índice
de um time e listar os jogadores que foram associados a ele.
"""
...
def print_teams():
    ...
    for i, team in enumerate(teams.values()):
        print(f"{i+1}. {team['name']} ({len(team['players'])} jogador(es))")
    print("\n")
...
while not done:
    ...
    if choice == "1":
        ...
        print_teams()
        team_num = int(input("Informa o número do time que deseja adicionar o jogador: \n"))
        if team_num <= len(teams):
            team_name = list(teams.keys())[team_num - 1]
        ...
        print_teams()
        team_num = int(input("Informa o número do time que deseja remover o jogador: \n"))
        if team_num <= len(teams):
            team_name = list(teams.keys())[team_num - 1]
        ...
        print_teams()
        team_name = int(input("Informe o número do time que deseja adicionar o jogador: \n"))
        if team_num <= len(teams):
            team_name = list(teams.keys())[team_num - 1]
            print_team_players(teams[team_name])
        else:
