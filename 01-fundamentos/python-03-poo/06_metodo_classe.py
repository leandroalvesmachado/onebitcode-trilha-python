"""
Utilizando Método de Classe.

1 - O método de classe utiliza o parâmetro referente a classe.
2 - O método de classe pode acessar ou modificar o estado da classe.
3 - Usamos o decorator @classmethod para criar um método de classe.

Recebe cls em vez de self.

Usado quando você quer:
    Criar objeto de forma alternativa
    Trabalhar com atributo da classe
"""

class Console:
    def __init__(self, name, price):
        self.name = name
        self.price = price
    
    @classmethod
    def from_text(cls, string):
        import re
        item = re.findall(r"é \w*", string)
        name = item[0][2:]
        price = item[1][2:]
        return cls(name, int(price))

# Não precisa instanciar o objeto como ta no construtor, pode aplicar o metodo diretamente
wii = Console.from_text("Meu video game é WiiU e o preço é 1000 reais")

print(wii.__dict__)  # {'name': 'WiiU', 'price': 1000}
